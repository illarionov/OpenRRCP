...
void do_restrict_rrcp(int port){
    union {
        struct {
            unsigned short low;
            unsigned short high;
    ...
    if(port==0){//do not disable any ports from rrcp
	u.signlelong=0;
    }else{//disable all but specific port from rrcp
	u.signlelong=(1<<(map_port_number_from_logical_to_physical(port)))^0xffffffff;
    }
...
void do_load_config(char *fname){
    FILE *f;
    char buf[1024];
    char *param=buf;
    char *value;
...
#define mode_port_counters 4
	...
	if(strcmp(buf,"[global]")==0){
	    mode=mode_global;
	}else if(strcmp(buf,"[vlan]")==0){
	    mode=mode_vlan;
	}else if(strcmp(buf,"[ports]")==0){
	    mode=mode_ports;
	}else if(strcmp(buf,"[port-counters]")==0){
	    mode=mode_port_counters;
	}else if(buf[0]=='['){
	    mode=0;
	}else if(buf[0]==';'){
	}else{
		...
		value=p+1;
		if (mode==mode_global){
		    if(strcmp(param,"restrict-rrcp-port")==0){
			do_restrict_rrcp(atoi(value));
		    }
		}else if (mode==mode_vlan){
		    if(strcmp(param,"hvlanroot")==0){
			do_vlan_enable_vlan(0,atoi(value),100);
		    }
		    if(strcmp(param,"8021qvlanroot")==0){
			do_vlan_enable_vlan(1,atoi(value),100);
		    }
		}else if (mode==mode_ports){
		}else if (mode==mode_port_counters){
		}
	    }
	}
    }
...
int main(int argc, char **argv){
	...
	do_softreboot();
    }else if(strcmp(argv[2],"load-config")==0){
	do_load_config(argv[3]);
    }else if(strcmp(argv[2],"restrict-rrcp")==0){
	do_restrict_rrcp(atoi(argv[3]));
